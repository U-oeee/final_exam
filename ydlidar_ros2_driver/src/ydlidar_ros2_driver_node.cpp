...
int main(int argc, char *argv[]) {
  ...
  if (ret) {
    RCLCPP_INFO(node->get_logger(), "YDLIDAR initialized");
  } else {
    RCLCPP_ERROR(node->get_logger(), "YDLIDAR initialization failed: %s", laser.DescribeError());
    rclcpp::shutdown();
    return -1;
  }
  ...
  ret = laser.turnOn();
  if (!ret) {
    RCLCPP_ERROR(node->get_logger(), "YDLIDAR TurnOn failed: %s", laser.DescribeError());
  }

  // ===== QoS 수정: SensorDataQoS + depth 5 =====
  auto qos = rclcpp::SensorDataQoS().keep_last(5).reliable();
  auto laser_pub =
