...
기능 요약
1) /cmd_vel (geometry_msgs/Twist) 구독
   - msg.linear.x  : 전/후진 속도 [m/s]
   - msg.angular.z : 회전 속도 [rad/s]
2) 차동구동 모델로 왼/오 바퀴 목표 속도 계산
3) 각 바퀴별 PID 속도 제어 + DIR 핀으로 방향 제어
4) 끊김 완화를 위한 처리:
   - 제어 주기 30 Hz
...
class PID:
    ...
    def update(self, target, measurement, dt):
        """
        target, measurement : 속도 [m/s] (양수 값)
        out : PWM duty [%] (0~100)
        """
...
# ==================== 메인 노드 ====================
class RB35MotorCmdVelNode(Node):
    def __init__(self):
        super().__init__('motor_pid_cmdvel')
        ...
        self.declare_parameter('wheel_base', 0.25)        # [m] 바퀴 간 거리 (대략값)
        ...
        self.get_logger().info(
            f"Node started (control_hz={control_hz})"
        )
        self.get_logger().info(
        ...
        # /cmd_vel에서 들어온 명령
        self.cmd_linear  = 0.0   # [m/s]
        self.cmd_angular = 0.0   # [rad/s]
    ...
    def setup_plot(self):
        ...
        # 속도 그래프
        self.ax_speed.set_title("Speed control (cmd vs left/right)")
        ...
        # 히스토리 버퍼
        self.time_hist = []
        self.target_speed_hist = []   # 목표 속도 크기 (max(|vL_cmd|, |vR_cmd|))
        self.left_speed_hist = []     # 왼쪽 모터 속도 크기
        self.right_speed_hist = []    # 오른쪽 모터 속도 크기
        self.left_pwm_hist = []       # 왼쪽 PWM
        self.right_pwm_hist = []      # 오른쪽 PWM

        # 선 객체
        (self.line_target_speed,) = self.ax_speed.plot([], [], 'g--', label='target |v|')
        (self.line_left_speed,)   = self.ax_speed.plot([], [], 'b-', label='left |v|')
        (self.line_right_speed,)  = self.ax_speed.plot([], [], 'r-', label='right |v|')
    ...
    def control_loop(self):
        ...
        # 차동 주행 모델
        v_left_cmd  = v - (w * self.wheel_base / 2.0)
        v_right_cmd = v + (w * self.wheel_base / 2.0)

        tgtL_abs = abs(v_left_cmd)
        tgtR_abs = abs(v_right_cmd)

        # 완전 정지 명령이면 PID reset + 모터 OFF
        if tgtL_abs < 1e-3 and tgtR_abs < 1e-3:
            self.pid_left.reset()
            self.pid_right.reset()
            self.current_pwm_left  = 0.0
            self.current_pwm_right = 0.0
            self.apply_pwm(0.0, 0.0, 0, 0)
            if self.debug:
                self.get_logger().info(
                    "[STOP] cmd_linear, cmd_angular ~ 0"
                )
            if self.plot_flag:
                self.update_plot(vL_abs, vR_abs, 0.0)
            return
        ...
        if tgtL_abs > 0.0:
            raw_pwm_left = self.pid_left.update(tgtL_abs, vL_abs, self.dt)
            if raw_pwm_left > 0.0:
        ...
        if tgtR_abs > 0.0:
            raw_pwm_right = self.pid_right.update(tgtR_abs, vR_abs, self.dt)
        ...
        if self.debug:
            self.get_logger().info(
                f"[PID] cmd_v=({v_left_cmd:.3f}, {v_right_cmd:.3f}) "
                f"|vL|={vL_abs:.4f}, |vR|={vR_abs:.4f}, "
        ...
        # ========== 5) 그래프 ==========
        if self.plot_flag:
            target_abs = max(tgtL_abs, tgtR_abs)
            self.update_plot(vL_abs, vR_abs, target_abs)

    # ---------- 그래프 업데이트 ----------
    def update_plot(self, v_left_abs, v_right_abs, target_abs):
        t_now = time.time() - self.plot_start_time
        self.time_hist.append(t_now)

        self.target_speed_hist.append(target_abs)
        self.left_speed_hist.append(v_left_abs)
        self.right_speed_hist.append(v_right_abs)
    ...
    # ---------- 모터 출력 ----------
    def apply_pwm(self, pwm_left: float, pwm_right: float, dir_left: int, dir_right: int):
        """
        pwm_left, pwm_right : 0~100 [%]
        dir_left, dir_right : -1 (역회전), 0 (정지), 1 (정회전)
        ...
        # 오른쪽 모터
        if dir_right == 0 or pwm_right <= 0.0:
            GPIO.output(MOTOR2_EN, GPIO.LOW)
